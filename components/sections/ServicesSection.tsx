...
export default function ServicesSection() {
  return (
    <section id="services" style={{ padding: "6rem 0", backgroundColor: "var(--bg-primary)" }}>
      <div style={{ maxWidth: "1280px", margin: "0 auto", padding: "0 1.5rem" }}>
        ...
        >
          <div style={{ width: "fit-content", marginBottom: "2.5rem" }}>
            <Typography variant="h2" style={{ marginBottom: "0.75rem" }}>
              <Typography variant="span" className="cyber-sweep">OUR</Typography> <Typography variant="span" className="cyber-sweep-red">SERVICES</Typography>
            </Typography>
          ...
          >
            From concept to post-launch support, we provide full-cycle development services powered by innovation.
          </Typography>
        </motion.div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "1.5rem", marginBottom: "5rem" }}>
          {services.map((s, i) => (
            ...
            >
              ...
              <Typography variant="h6" style={{ color: "var(--text-primary)", marginBottom: "0.75rem" }}>{s.title}</Typography>
              <Typography variant="b3" style={{ color: "var(--text-secondary)", lineHeight: 1.65, textTransform: "none" }}>{s.text}</Typography>
            </motion.div>
          ))}
        </div>
        ...
        <div style={{ marginTop: "6rem" }}>
          <Typography variant="h4" style={{ color: "var(--text-primary)", marginBottom: "2.5rem", textAlign: "center" }}>
            Technical <Typography variant="span" style={{ color: "var(--accent-red)" }}>Expertise</Typography>
          </Typography>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: "2rem" }}>
             {reels.map((r) => (
               <div key={r.title} style={{ padding: "1.5rem", borderLeft: "2px solid var(--accent-red)", background: "rgba(255,255,255,0.01)" }}>
                  <Typography variant="h6" style={{ fontSize: "1rem", color: "var(--text-primary)", marginBottom: "0.5rem" }}>{r.title}</Typography>
                  <Typography variant="b3" style={{ fontSize: "0.85rem", color: "#666", lineHeight: 1.6, textTransform: "none" }}>{r.text}</Typography>
               </div>
             ))}
          </div>
        </div>
